					...
					<div class="col-lg-4">
							...
							<div class="col-md-12">
								<div class="cart-detail">
									...
									<ul>
										<li>
											<ul>
												<li><span>${gname } / ${goodscountDTO.gstock } 개</span> <span>+ ${sum }</span></li>  <!-- 주문하는 상품들 -->
												<li><span><sup>${goodscountDTO.gcolor } / ${goodscountDTO.gsize }</sup></span></li>
											</ul>
										</li>
										...
										<li><span>사용할 마일리지</span><span><input type="text" class="form-control" name="m_mileage" id="m_mileage" disabled="disabled" style="height: 25px !important; "></span>
						...
						<div class="row">
							<div class="col-md-12 text-center">
								<p><input type="button" value="주문하기" onclick="directPurchase()" class="btn btn-primary"></p>  <!-- 수정 -->
							</div>
						</div>
					</div>
				</div>
...
function directPurchase() {
	...
	var memdirect = $('input:radio[name=memdirect]:checked').val();
	// 테이블 추가를 위한 정보
	var gcode = '${goodscountDTO.gcode}';
	var gcolor = '${goodscountDTO.gcolor}';
	var gsize = '${goodscountDTO.gsize}';
	var gstock = '${goodscountDTO.gstock}';
	var gname = '${gname}'
	...
	if(authority == 3) {
		...
		IMP.request_pay({
		    ...
		    if ( rsp.success ) {
		    	var msg = '결제가 완료되었습니다.';
				location.href="directbuySuccess?gcode="+gcode+"&gname="+gname+"&gcolor="+gcolor+"&gsize="+gsize+"&o_quantity="+gstock+"&mid="+id+"&mname="+name+"&mgender="+mgender+"&o_age="+o_age+"&o_phone="+phone+"&o_add="+madd+"&o_request="+o_request+"&o_gprice="+gprice+"&mileage="+mileage;
		    } else {
		    	 var msg = '결제에 실패하였습니다.\n';
